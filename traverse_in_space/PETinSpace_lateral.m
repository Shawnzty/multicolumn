...
% container
PET = zeros(Delta_steps, 16, 16, 6);
finalR = zeros(Delta_steps, 16, 6);
PET_tmp = zeros(16,16,6);
...
parfor (n =1:Delta_steps,10)
    ...
    PETC = getPET(v(:,strt_prd:end_prd,:), g(:,:,strt_prd:end_prd,:)); % power in unit of micro watt (\mu W), size: 16*16*time*5
    PET0 = getPET(v(:,50001:100000,:),g(:,:,50001:100000,:));
    PET(n,:,:,:) = cat(3,PET0(:,:,1), PETC);

    disp(etime(clock, startTime));
end

filename = append('../../data/','PETN_',num2str(Delta_e_start),'_',num2str(Delta_e_end),'_',...
    num2str(Delta_i_start),'_',num2str(Delta_i_end), '_Iattn_', num2str(Iattn), '.mat');
save(filename,'PET','finalR'); 
