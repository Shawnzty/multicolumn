...
for n = 1:Delta_steps
    ...
    [strt_prd, end_prd] = getOnePrd(squeeze(r(5,end-40000:end,4)));
    strt_prd = strt_prd + size(r,2) - 40000; end_prd = end_prd + size(r,2) - 40000;
    peTrans = squeeze(sum(pp(:,:,strt_prd:end_prd,:),3)).*p*0.001*0.01; % 16*16*5
    PET(n,:,:,:) = peTrans;
    disp(etime(clock, startTime));
end

filename = append('onePrd_',num2str(Delta_e_start),'_',num2str(Delta_e_end),'_',...
