% PRV is the combination of both firing rate "r" and menbrane potential "v"
clear;
close all;
initime = clock;
addpath('funcs');

% get the change of PRV in parameter space
...
for n = 1:Delta_steps
    ...
    prv = getPRV(r,v,g,strt_prd,end_prd); % current in unit of micro A (\mu A?), size: 16*16*time*5
    PRV(n,:,:,:) = prv;
    disp(etime(clock, startTime));
end

filename = append('PRV_',num2str(Delta_e_start),'_',num2str(Delta_e_end),'_',...
